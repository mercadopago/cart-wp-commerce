...
include_once "lib/MPApi.php";
	...
	function mp_retorno(){
		...
		if (isset($_REQUEST['collection_id'])) {
			...
			$checkdata = New MP($client_id,$client_secret);
			
			$sandbox = get_option('mercadopago_sandbox');
			if($sandbox){
				$checkdata->sandbox_mode(true);
			}

			$dados = $checkdata->get_payment_info($id);
			
			$order_id = $dados['response']['collection']['external_reference'];
			$order_status = $dados['response']['collection']['status'];
			$mp_id = $dados['response']['collection']['order_id'];
	...
	function mp_results(){
		// Function used to translate the MercadoPago returned external_reference=sessionid GET variable into the recognised GET variable for the transaction results page.
		if(isset($_REQUEST['external_reference']) && ($_REQUEST['external_reference'] !='') && ($_GET['sessionid'] == '')){
			$_GET['sessionid'] = $_REQUEST['external_reference'];
		}
	}

add_action('init', 'mp_retorno');
add_action('init', 'mp_results');
